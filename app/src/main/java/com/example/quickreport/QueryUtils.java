...
public class QueryUtils {
    ...
    public static ArrayList<earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray earthArray = baseJsonResponce.getJSONArray("features");
            for (int i=0 ; i< earthArray.length() ; i++){
               JSONObject currentEarthqueak = earthArray.getJSONObject(i);
               JSONObject properties = currentEarthqueak.getJSONObject("properties");

               double mag = properties.getDouble("mag");
               //modify it to get only 2 digit of mod
                DecimalFormat formatter = new DecimalFormat("0.00");
                String output = formatter.format(mag);

               String cityname = properties.getString("place");
                //break the cityname in two parts
                String smallcity = "Near the";
                String bigcity  = cityname;

                if (cityname.contains(" of ")){
                String[] all = cityname.split(" of ",4);
                 smallcity = all[0];
                 bigcity  = all[1];}


                //convert time from no to a fix formate
               long time = properties.getLong("time");
                ...
                earthquake earthquake = new earthquake(mag,smallcity,bigcity,dateToDisplay,url);
